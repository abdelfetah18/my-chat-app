    ...
    return (
        <div className="flex flex-col items-center lg:w-1/6 md:w-1/4 w-full mb-8">
            <div className="flex flex-col w-11/12 shadow-lg items-center rounded-lg">
                ...
                <div className="font-mono font-semibold text-purple-800 text-base mt-2 w-full px-2">{room.name}</div>
                <div className="font-mono text-purple-400 text-xs mb-2 w-full px-2">{room.total_members} Members</div>
                <div className="w-full px-2 my-2">
                    <SubmitButton key={crypto.randomUUID()} wrapperClassName={''} onClick={onClick} text={"JOIN"} className="bg-purple-600 text-blue-50 text-sm w-full justify-center py-2" />
                </div>
            </div>
        </div>
    );
