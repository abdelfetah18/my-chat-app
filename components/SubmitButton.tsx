...
export default function SubmitButton({ text, Icon=null, onClick, className, wrapperClassName } : SubmitButtonProps){
    ...
    return (
        <div className={"flex flex-col items-center "+wrapperClassName}>
            ...
            <div onClick={click} className={"flex items-center font-mono font-semibold rounded-lg cursor-pointer px-8 py-1 hover:bg-purple-700 hover:text-purple-50 "+className}>{ Icon && <Icon className="mr-2" />} {text}</div>
        </div>
    )
}