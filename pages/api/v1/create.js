...
export default function handler(req, res) {
    ...
    getData('*[_type=="rooms" && name==$room_name]',{ room_name }).then((rooms) => {
        if(rooms.length > 0){
            ...
            let room_doc = { _type:'rooms',name:room_name,bio:room_bio,admin:{ _ref:user_info.user_id },creator:{ _ref:user_info.user_id }};
            addData(room_doc).then((room) => {
                let member_doc = { _type:'room_members', room:{ _ref:room._id }, member:{ _ref:user_info.user_id }, role:"admin", state:"accept" };
