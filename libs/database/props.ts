...
export const USER_PROPS = `{ _id, "username": username.current, "profile_image": profile_image.asset->${ASSET_PROPS}, "cover_image": cover_image.asset->${ASSET_PROPS}, bio, _createdAt, "rooms": count(*[_type=="room_member" && user._ref==^._id]), "friends": count(*[_type=="friend" && (user._ref==^._id || inviter._ref==^._id) && state=="accept"]) }`;
